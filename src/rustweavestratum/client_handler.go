...
func (c *clientListener) NewBlockAvailable(kapi *rustweaveApi) {
	c.clientLock.Lock()
		...
		go func(client *gostratum.StratumContext) {
			...
			if err != nil {
				if strings.Contains(err.Error(), "Could not decode address") {
					...
					client.Logger.Error(fmt.Sprintf("failed fetching new block template from rustweave, malformed address: %s", err))
					client.Disconnect() // unrecoverable
				} else {
					RecordWorkerError(client.WalletAddr, ErrFailedBlockFetch)
					client.Logger.Error(fmt.Sprintf("failed fetching new block template from rustweave: %s", err))
				}
				return
			}
			...
			if !state.initialized {
				...
				state.stratumDiff = newrustweaveDiff()
	...
	if time.Since(c.lastBalanceCheck) > balanceDelay {
		...
		if len(addresses) > 0 {
			go func() {
				balances, err := kapi.rustweaved.GetBalancesByAddresses(addresses)
				if err != nil {
					c.logger.Warn("failed to get balances from rustweave, prom stats will be out of date", zap.Error(err))
					return
				}
